...
def write_jsonlist(list_of_json_objects, output_filename, sort_keys=True, do_gzip=False):
    if do_gzip:
        with gzip.open(output_filename, 'w') as output_file:
...
def read_text(input_filename, encoding='utf-8'):
    with codecs.open(input_filename, 'r', encoding=encoding) as input_file:
        lines = input_file.readlines()
    return lines


def write_list_to_text(lines, output_filename, add_newlines=True, add_final_newline=False, do_gzip=False):
    if add_newlines:
        lines = '\n'.join(lines)
